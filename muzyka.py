...
name = ['Roses - prod44.one','лазить по стенам - cenker','they are dead - DJ UNIVXRSEL','stay_w_me original - m1v',
        ...
        '26 Sezon 3 chapter 3', '26 Sezon 4 chapter 1', '26 Sezon 4 chapter 2', '26 Sezon 5 chapter 1', '26 Sezon 5 chapter 2', 
        '26 Sezon 5 chapter 3', '26 Sezon 6 chapter 1', '26 Sezon 6 chapter 2', '26 Sezon 6 chapter 3', 
        f"Lumyn's Mixtape"]
name = name[:-1]
print(len(name))
...
special = []
### AKA THE TIME FRAME
time_frame = np.arange(len(name))+1
pull = 1 / (1 + (time_frame / 1) ** 2)
pull = pull / np.sum(pull) * k
force = np.zeros(len(name))
force[-1] = pull[-1] * time_frame[-1]
for i in range(len(name)-1):
    done = np.sum(force[-1-i:] / time_frame[-1-i:])
    force[-2-i] = (pull[-2-i] - done) * time_frame[-2-i]
#force = np.exp(-force / 2)
print(force)
input("force")
helper = np.ones(force.shape) * 0.0
while len(special) < k:
